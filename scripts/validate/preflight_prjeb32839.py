#!/usr/bin/env python3
"""Scientific preflight for the frozen PRJEB32839 inventory and DE design."""
...
EXPECTED_RUNS = 150
EXPECTED_SAMPLES = 75
EXPECTED_RUNS_PER_SAMPLE = 2
EXPECTED_ENA_RUNS = 180
EXPECTED_EXCLUDED_RUNS = 30
EXPECTED_STUDY = "PRJEB32839"
EXPECTED_SECONDARY_STUDY = "ERP115565"
...
def validate(root: Path, ena_path: Path, output: Path) -> dict[str, object]:
    ...
    _, excluded = read_table(metadata_dir / "excluded_ena_runs.tsv")
    ...
    add_error(errors, len(ena) != EXPECTED_ENA_RUNS, f"ENA run count is {len(ena)}, expected {EXPECTED_ENA_RUNS}")
    add_error(errors, len(excluded) != EXPECTED_EXCLUDED_RUNS, f"excluded run count is {len(excluded)}, expected {EXPECTED_EXCLUDED_RUNS}")
    ...
    excluded_counts = Counter(row["run_accession"] for row in excluded)
    ...
    duplicated_excluded = sorted(run for run, count in excluded_counts.items() if count != 1)
    ...
    add_error(errors, bool(duplicated_excluded), f"excluded duplicate run accessions: {duplicated_excluded[:10]}")
    ...
    excluded_by_run = {row["run_accession"]: row for row in excluded}
    frozen_set, metadata_set, ena_set, excluded_set = set(frozen_by_run), set(metadata_by_run), set(ena_by_run), set(excluded_by_run)
    add_error(errors, frozen_set != metadata_set, "frozen runs and workflow metadata contain different run accessions")
    add_error(errors, not frozen_set.issubset(ena_set), "one or more frozen runs are absent from live ENA")
    add_error(errors, frozen_set & excluded_set, "included and excluded run inventories overlap")
    add_error(errors, frozen_set | excluded_set != ena_set, "included plus excluded inventories do not equal live ENA")
    ...
    for sample_id in sorted(sample_by_id):
        ...
        expected_units = [f"lane_{index}" for index in range(1, EXPECTED_RUNS_PER_SAMPLE + 1)]
    ...
    gates = {
        "frozen_run_inventory": "PASS" if frozen_set == metadata_set and frozen_set | excluded_set == ena_set and not frozen_set & excluded_set else "FAIL",
        "run_to_sample_mapping": "PASS" if not conflicts and not duplicated_runs and frozen_set == metadata_set and frozen_set.issubset(ena_set) else "FAIL",
        ...
        "ena_reconciliation": "PASS" if not ena_differences and frozen_set | excluded_set == ena_set else "FAIL",
    ...
    report: dict[str, object] = {
        ...
        "run_inventory": {
            ...
            "orphan_runs": len(ena_set - frozen_set - excluded_set),
        },
        "sample_accounting": {

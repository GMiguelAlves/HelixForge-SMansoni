...
config="$HF_PACKAGE_ROOT/studies/$project/pipeline_config.sh"
outdir="$HF_RESULTS_ROOT/$project"
args=(run "$HF_HELIXFORGE_ROOT" -profile "${HF_PROFILES:-apptainer,slurm}" -c "$HF_PACKAGE_ROOT/site/slurm.config" -w "$HF_WORK_ROOT" -resume --workflow rnaseq --rnaseq_run_mode "$mode" --rnaseq_analysis_mode quantification --rnaseq_config "$config" --rnaseq_import_policy production_v1 --rnaseq_library_protocol full_length --rnaseq_counts_from_abundance lengthScaledTPM --salmon_validate_mappings true --rnaseq_report_enabled false --outdir "$outdir" -with-report "$outdir/execution_report.html" -with-trace "$outdir/trace.tsv" -with-timeline "$outdir/timeline.html" -with-dag "$outdir/dag.html")
if [[ "$mode" == full ]]; then args+=(--rnaseq_de_spec "$HF_PACKAGE_ROOT/studies/$project/de_spec.json"); fi
"$nextflow_bin" "${args[@]}"
